...
async function runSecurityOperatorVerification() {
  ...
  const visit = await prisma.vehicleVisit.create({
    data: {
      ...
      portions: {
        create: [
          { portion_number: 1, dispatch_quantity_value: 5000, dispatch_quantity_unit: 'KG', dispatch_quantity_basis: 'MEASURED', current_status: 'DISPATCHED' },
          { portion_number: 2, dispatch_quantity_value: 4000, dispatch_quantity_unit: 'KG', dispatch_quantity_basis: 'MEASURED', current_status: 'DISPATCHED' },
        ],
      },
    },
