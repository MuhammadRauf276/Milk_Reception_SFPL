...
export function serializeMotArrival(arrival: any) {
  return {
    ...
    identifiers: paperLinkedIdentity(
      { entity: 'zmcc_mot_arrival', id: arrival.id.toString(), number: arrival.zmcc_token },
      [{ type: 'RAW_MILK_TOKEN', value: arrival.raw_milk_token_number || null }]
    ),
...
export function serializeLocalSupplierArrival(arrival: any) {
  return {
    ...
    raw_milk_token_number: arrival.raw_milk_token_number ?? null,
    identifiers: paperLinkedIdentity(
      { entity: 'zmcc_local_supplier_arrival', id: arrival.id.toString(), number: arrival.zmcc_token },
      [
        { type: 'RAW_MILK_TOKEN', value: arrival.raw_milk_token_number ?? null },
        { type: 'LEGACY_LOCAL_SUPPLIER_RMR', value: arrival.rmr_number ?? null },
      ]
    ),
    vehicle_number: arrival.vehicle_number,
    arrival_timestamp: arrival.arrival_timestamp instanceof Date ? arrival.arrival_timestamp.toISOString() : arrival.arrival_timestamp,
...
export async function submitLocalSupplierArrival(
  ...
  if (!normalizePakistaniMobile(localSupplier.phone).ok || !normalizePakistaniCnic(localSupplier.cnic).ok) {
    return { status: 400, error: 'Selected local supplier has incomplete phone/CNIC identity and cannot receive new milk arrivals until a ZMCC Manager corrects the record.' };
  }
